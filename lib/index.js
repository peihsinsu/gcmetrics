...
exports.init = init;
function init(opts) {
	log.debug('init with opts: ', opts);
	auth.init(opts);
}

exports.generateTSData = generateTSData;
function generateTSData(project, name, value, start, end, labels) {
...
function createMetricDescriptor(project, name, labels, callback) {
  var data = {
	 ...
	 "description": "Sales by store and date.",
	 "typeDescriptor": {
		"metricType": "gauge",
		"valueType": "double"
	 },
...
function writeTimeSeries(project, name, value, start, end, labels, callback) {
	var opts = {
		url: util.format(url.writeTimeSeries, project),
		json: generateTSData(project, name, value, start, end, labels),
		method: 'POST'
	};
  log.debug('create timeseries metroc with:', JSON.stringify(opts));	
	request(opts, callback);
}
...
function getMetric(projectId, name, callback) {
   request({
    url: util.format(url.getMetric, projectId, 
					 encodeURIComponent('custom.cloudmonitoring.googleapis.com/' + name)),
		method: 'GET',
		qs: {
      youngest: new Date(new Date().getTime() - 1*24*60*60*1000).toISOString()
		}
	}, callback);
}
